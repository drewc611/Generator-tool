...
/**
 * The same page written in jinja, Twig, Liquid, Blade, FreeMarker, Velocity, Thymeleaf, Smarty, JSP, CFML, Haml, Slim, Twirl, Django, EJS and Nunjucks produces one React
...
const DIALECTS = ["jinja", "twig", "liquid", "blade", "freemarker", "velocity", "thymeleaf", "smarty", "jsp", "cfml", "haml", "slim", "twirl", "django", "ejs", "nunjucks"];
const PROVENANCE = /^.*(Ported from|Template translated from|<!-- (Ported|Translated)|Source: ).*$\n?/gm;
const strip = (text) => text.replace(PROVENANCE, "");

test("one page in sixteen server dialects is one component in each target, provenance lines aside", async () => {
