...
/**
 * The same page written in jinja, Twig, Liquid, Blade, FreeMarker, Velocity and Thymeleaf produces one React
...
const DIALECTS = ["jinja", "twig", "liquid", "blade", "freemarker", "velocity", "thymeleaf"];
const PROVENANCE = /^.*(Ported from|Template translated from|<!-- (Ported|Translated)|Source: ).*$\n?/gm;
const strip = (text) => text.replace(PROVENANCE, "");

test("one page in seven server dialects is one component in each target, provenance lines aside", async () => {
