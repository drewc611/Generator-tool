...
**683. app.html is rebuilt whole, three times, into a design studio's own console: rounded, floating cards on a pale workspace, one restrained accent marking a selection and the primary button and nothing else, a pill shaped segmented control for the inspector's faces, a media scrubber between the recorded screenshot and the emitted source, and a real mobile app screen below the desk's own breakpoint, a bottom tab bar and a floating action button swapping the same three panels one at a time** 🔨
A person asked for the console rebuilt entirely, unlike anything out there, twice more after the first attempt: a ledger register (ruled paper, a serif hand) was rejected in full; a native, Apple-system console (frosted blurred panels, a macOS segmented control) that followed was named "still looks like VS Code" and pointed at Canva mixed with Photoshop instead, with a mobile app alongside it. The third pass drops every blur: the sidebar, the main pane and the inspector are each their own rounded card with a soft shadow on a neutral workspace, the way a design tool's canvas sits on its table, rather than a flush edge to edge dock; section headers and the wordmark take a rounded system face (`ui-rounded`) while code and paths keep the monospace stack; verb and status columns are colored pills. It first shipped with a violet into pink gradient carrying the accent everywhere — the brand dot, every badge, every section header — named hideous once seen live; the gradient is dropped for one restrained blue that marks only a selection state and the primary button, because an accent spent on six things marks nothing. Asked again for more blackness and more blue, night's neutral dark gray moves to a near black with a blue cast running through the workspace, the panels and every hairline, and becomes the default outright rather than one two systems could pick between, because the console's own content is screenshots and code; day is retinted to match, a pale blue-gray table in place of its earlier lavender one, and still sits one key away. Below 980px the same three panels — the same DOM, not a second copy — become full mobile screens switched by a bottom tab bar (Screens, Compare, Signals, Intake), with a floating action button calling the same `rerun()` the sidebar's button already does; the console's manifest already declared `"display": "standalone"`, so the home-screen install this asks for was one line of CSS and markup away, not a new capability. lib.js and index.js are untouched across all three passes; every id, class and literal fragment test/ui.test.js already held the page to (the aria-pressed flag filter, the FLAG_TITLE block, the tooltip's exact CSS, the intake's needles) survives structurally in the new markup, so each rebuild is provably the same tool wearing a different skin rather than a page that merely looks different. Real defects surfaced building the three passes and are fixed rather than carried forward: a flex item's default min-width had been letting a search input overflow its own column instead of shrinking; an empty-state message with no clip printed over the code view behind it whenever a screenshot turned out to be an unreadable placeholder; the mast's readout line had no truncation and wrapped onto the sidebar below it at narrow widths; the sidebar's own middle sections, with no `overflow: hidden` of their own, spilled their header and filter row into a neighbouring section instead of clipping when a short viewport left them no room to shrink into; a mobile view-switching rule sat earlier in the stylesheet than an equal specificity desktop rule and so never won, showing all three panels at once instead of one; and the mobile sidebar tried to give each of its lists its own independent scroll the way the desktop dock does, squeezing the shortest one to nothing instead of letting the whole screen scroll as one page. The line budget, raised from 2000 to 2050 for the second pass and to 2150 for the third, holds with room to spare; it is docs/UI-SPEC.md's own number too, and the README's console images are regenerated as real screenshots of the console the run actually serves rather than a hand-drawn mockup of the skin it replaced. test/ui.test.js's existing thirty four cases hold the contract; none needed rewriting.

## Phase 187: the console's own content gets the same care its shell already had
...
## Phase 195: the other half of full stack
...
Since it first shipped, the gate that keeps a URL out of an emitted component ran at verify, after the whole port was already written: it could fail the run and name the file, but the file was already on disk to look at, unlike the secret gate, which stops a credential from ever being copied out of the source that names it. Policy gains `assertComponentWrite`, called from `ctx.write` itself on every write rather than left to a plugin's own choice to check it: the write of a component naming a raw endpoint now throws before its bytes exist. The classification of what counts as a component, src/features, src/elements and src/app, minus the site shell's own data modules, moves onto `policy.isComponentPath`, so the write time check and the verify stage scan kept as the second net for anything that reaches disk some other way can no longer disagree by accident about which tree either one means. Moving the check earlier turned out not to need checking a component before it exists: every plugin that collects endpoints and routes runs at plan, every plugin that writes a component runs at emit, and a staged pipeline finishes one stage before starting the next, so the map a component is checked against is already whole by the time any component is written. test/pipeline.test.js now asserts the refused component never reaches disk, not only that the run stopped, and test/policy.test.js holds isComponentPath and assertComponentWrite directly.
