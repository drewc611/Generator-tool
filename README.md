...
lines of core, no runtime dependencies, 212 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 1224 passing, 1225 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the executable reader, the measured screenshot and the console's intake](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 1224
tests pass across 160 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 212 plugin(s)
...
| Every line of the tool | 57,720 lines of JavaScript in src and plugins |
| Tests | 21,213 lines, 1225 cases across 160 files |
| Source on disk | src 27 KB, plugins 2.7 MB |
...
The core grew from 527 lines to 718 across six hundred and seventy one features, and every
...
## The 212 it ships with

![The plugin rack: 212 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and seventy one features in
one hundred and seventy four phases, forty four shipped, six hundred and twenty four new in the
