...
lines of core, no runtime dependencies, 216 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 1293 passing, 1294 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the executable reader, the measured screenshot and the console's intake](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 1293
tests pass across 164 test files with `node --test` and no framework, and
...
carries a suppression comment saying so. CI reruns the same
...
node src/cli.js plugins      # 216 plugin(s)
...
| Every line of the tool | 59,782 lines of JavaScript in src and plugins |
| Tests | 22,221 lines, 1294 cases across 164 files |
| Source on disk | src 27 KB, plugins 2.8 MB |
...
The core grew from 527 lines to 718 across six hundred and seventy five features, and every
...
## The 216 it ships with

![The plugin rack: 216 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and seventy five features in
one hundred and seventy eight phases, forty four shipped, six hundred and twenty eight new in the
