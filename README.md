...
lines of core, no runtime dependencies, 92 plugins, and the literal output of
`npm run demo`.</sub>
...
node src/cli.js plugins      # 92 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 532 tests, node --test, no framework
```
...
| Every line of the tool | 21,616 lines of JavaScript |
| Tests | 6,471 lines, 532 cases |
...
![The plugin rack: 92 plugins listed by class, with what each one does](media/plugin-rack.svg)
