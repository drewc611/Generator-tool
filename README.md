...
lines of core, no runtime dependencies, 172 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 848 passing, 849 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 848
tests pass across 111 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 172 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 849 tests, node --test, no framework
```
...
| Tests | 11,904 lines, 849 cases across 111 files |
...
The core grew from 527 lines to 718 across six hundred features, and every
...
## The 172 it ships with

![The plugin rack: 172 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred features in
one hundred and sixteen phases, forty four shipped, five hundred and fifty three new in the
