...
![node --test running the portamp suite: 876 passing, 877 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 876
...
npm test                     # 877 tests, node --test, no framework
```
...
| Tests | 12,429 lines, 877 cases across 115 files |
...
The core grew from 527 lines to 718 across six hundred and seven features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and seven features in
one hundred and twenty three phases, forty four shipped, five hundred and sixty new in the
