...
![node --test running the portamp suite: 924 passing, 925 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 924
tests pass across 122 test files with `node --test` and no framework, and
...
npm test                     # 925 tests, node --test, no framework
```
...
| Every line of the tool | 41,201 lines of JavaScript in src and plugins |
| Tests | 13,541 lines, 925 cases across 122 files |
...
The core grew from 527 lines to 718 across six hundred and twenty two features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and twenty two features in
one hundred and thirty eight phases, forty four shipped, five hundred and seventy five new in the
