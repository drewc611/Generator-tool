...
lines of core, no runtime dependencies, 179 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 913 passing, 914 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 913
tests pass across 120 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 179 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 914 tests, node --test, no framework
```
...
| Every line of the tool | 40744 lines of JavaScript in src and plugins |
| Tests | 13289 lines, 914 cases across 120 files |
...
The core grew from 527 lines to 718 across six hundred and eighteen features, and every
...
## The 179 it ships with

![The plugin rack: 179 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and eighteen features in
one hundred and thirty four phases, forty four shipped, five hundred and seventy one new in the
