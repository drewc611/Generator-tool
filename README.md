...
lines of core, no runtime dependencies, 159 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 773 passing, 774 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit, Alpine and Stencil readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 773
tests pass across 96 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 159 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 774 tests, node --test, no framework
```
...
| Every line of the tool | 34,210 lines of JavaScript in src and plugins |
| Tests | 10,667 lines, 774 cases across 96 files |
...
The core grew from 527 lines to 718 across five hundred and seventy eight features, and every
...
## The 159 it ships with

![The plugin rack: 159 plugins listed by class, with what each one does](media/plugin-rack.svg)
