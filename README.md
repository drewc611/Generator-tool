...
lines of core, no runtime dependencies, 100 plugins, and the literal output of
`npm run demo`.</sub>
...
node src/cli.js plugins      # 100 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 546 tests, node --test, no framework
```
...
| Every line of the tool | 22,563 lines of JavaScript |
| Tests | 6,735 lines, 546 cases |
...
![The plugin rack: 100 plugins listed by class, with what each one does](media/plugin-rack.svg)
