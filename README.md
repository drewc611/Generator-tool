...
lines of core, no runtime dependencies, 107 plugins, and the literal output of
`npm run demo`.</sub>
...
node src/cli.js plugins      # 107 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 556 tests, node --test, no framework
```
...
| Every line of the tool | 23,290 lines of JavaScript |
| Tests | 6,945 lines, 556 cases |
...
![The plugin rack: 107 plugins listed by class, with what each one does](media/plugin-rack.svg)
