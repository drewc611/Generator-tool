...
![node --test running the portamp suite: 871 passing, 872 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 871
tests pass across 114 test files with `node --test` and no framework, and
...
| Tests | 12,310 lines, 872 cases across 114 files |
...
The core grew from 527 lines to 718 across six hundred and six features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and six features in
one hundred and twenty two phases, forty four shipped, five hundred and fifty nine new in the
