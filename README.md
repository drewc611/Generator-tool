...
![node --test running the portamp suite: 934 passing, 935 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 934
tests pass across 125 test files with `node --test` and no framework, and
...
npm test                     # 935 tests, node --test, no framework
```
...
| Every line of the tool | 41,584 lines of JavaScript in src and plugins |
| Tests | 13,781 lines, 935 cases across 125 files |
...
The core grew from 527 lines to 718 across six hundred and twenty eight features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and twenty eight features in
one hundred and forty three phases, forty four shipped, five hundred and eighty one new in the
