...
lines of core, no runtime dependencies, 113 plugins, and the literal output of
`npm run demo`.</sub>
...
node src/cli.js plugins      # 113 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 565 tests, node --test, no framework
```
...
| Every line of the tool | 24,117 lines of JavaScript |
| Tests | 7,172 lines, 565 cases |
...
![The plugin rack: 113 plugins listed by class, with what each one does](media/plugin-rack.svg)
