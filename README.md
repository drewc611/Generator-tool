...
lines of core, no runtime dependencies, 168 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 811 passing, 812 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 811
tests pass across 106 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 168 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 812 tests, node --test, no framework
```
...
| Every line of the tool | 36,030 lines of JavaScript in src and plugins |
| Tests | 11,381 lines, 812 cases across 106 files |
...
The core grew from 527 lines to 718 across five hundred and ninety features, and every
...
## The 168 it ships with

![The plugin rack: 168 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): five hundred and ninety features in
one hundred and six phases, forty four shipped, five hundred and forty three new in the
