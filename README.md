...
lines of core, no runtime dependencies, 164 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 797 passing, 798 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit, Alpine and Stencil readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 797
tests pass across 101 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 164 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 798 tests, node --test, no framework
```
...
| Every line of the tool | 35,296 lines of JavaScript in src and plugins |
| Tests | 11,089 lines, 798 cases across 101 files |
...
The core grew from 527 lines to 718 across five hundred and eighty four features, and every
...
## The 164 it ships with

![The plugin rack: 164 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): five hundred and eighty four features in
one hundred phases, forty four shipped, five hundred and thirty seven new in the
