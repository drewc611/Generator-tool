...
lines of core, no runtime dependencies, 166 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 805 passing, 806 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 805
tests pass across 103 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 166 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 806 tests, node --test, no framework
```
...
| Every line of the tool | 35,813 lines of JavaScript in src and plugins |
| Tests | 11,242 lines, 806 cases across 103 files |
| Source on disk | src 27 KB, plugins 1.5 MB |
...
The core grew from 527 lines to 718 across five hundred and eighty seven features, and every
...
## The 166 it ships with

![The plugin rack: 166 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): five hundred and eighty seven features in
one hundred and three phases, forty four shipped, five hundred and forty new in the
