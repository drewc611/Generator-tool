...
lines of core, no runtime dependencies, 174 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 864 passing, 865 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 864
tests pass across 113 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 174 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 865 tests, node --test, no framework
```
...
| Tests | 12,204 lines, 865 cases across 113 files |
...
The core grew from 527 lines to 718 across six hundred and three features, and every
...
## The 174 it ships with

![The plugin rack: 174 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and three features in
one hundred and nineteen phases, forty four shipped, five hundred and fifty six new in the
