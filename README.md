...
lines of core, no runtime dependencies, 160 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 781 passing, 782 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit, Alpine and Stencil readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 781
tests pass across 97 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 160 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 782 tests, node --test, no framework
```
...
| Every line of the tool | 34,610 lines of JavaScript in src and plugins |
| Tests | 10,774 lines, 782 cases across 97 files |
...
The core grew from 527 lines to 718 across five hundred and seventy nine features, and every
...
## The 160 it ships with

![The plugin rack: 160 plugins listed by class, with what each one does](media/plugin-rack.svg)
