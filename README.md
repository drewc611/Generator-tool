...
![node --test running the portamp suite: 834 passing, 835 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 834
tests pass across 109 test files with `node --test` and no framework, and
...
| Every line of the tool | 36,967 lines of JavaScript in src and plugins |
| Tests | 11,720 lines, 835 cases across 109 files |
...
The core grew from 527 lines to 718 across five hundred and ninety seven features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): five hundred and ninety seven features in
one hundred and thirteen phases, forty four shipped, five hundred and fifty new in the
