...
lines of core, no runtime dependencies, 173 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 856 passing, 857 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 856
tests pass across 112 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 173 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 857 tests, node --test, no framework
```
...
| Tests | 12,059 lines, 857 cases across 112 files |
...
The core grew from 527 lines to 718 across six hundred and one features, and every
...
## The 173 it ships with

![The plugin rack: 173 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and one features in
one hundred and seventeen phases, forty four shipped, five hundred and fifty four new in the
