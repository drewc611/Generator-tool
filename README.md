...
lines of core, no runtime dependencies, 220 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 1369 passing, 1370 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the executable reader, the measured screenshot and the console's intake](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 1369
tests pass across 168 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 220 plugin(s)
...
| Every line of the tool | 61,760 lines of JavaScript in src and plugins |
| Tests | 23,194 lines, 1370 cases across 168 files |
...
The core grew from 527 lines to 718 across six hundred and seventy nine features, and every
...
## The 220 it ships with

![The plugin rack: 220 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and seventy nine features in
one hundred and eighty two phases, forty four shipped, six hundred and thirty two new in the
