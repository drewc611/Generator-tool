...
lines of core, no runtime dependencies, 171 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 838 passing, 839 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 838
tests pass across 110 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 171 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 839 tests, node --test, no framework
```
...
| Every line of the tool | 36,912 lines of JavaScript in src and plugins |
| Tests | 11,759 lines, 839 cases across 110 files |
| Source on disk | src 27 KB, plugins 1.6 MB |
...
The core grew from 527 lines to 718 across five hundred and ninety eight features, and every
...
## The 171 it ships with

![The plugin rack: 171 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): five hundred and ninety eight features in
one hundred and fourteen phases, forty four shipped, five hundred and fifty one new in the
