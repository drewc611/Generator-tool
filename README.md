...
lines of core, no runtime dependencies, 218 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 1330 passing, 1331 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the executable reader, the measured screenshot and the console's intake](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 1330
tests pass across 166 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 218 plugin(s)
...
| Every line of the tool | 60,755 lines of JavaScript in src and plugins |
| Tests | 22,698 lines, 1331 cases across 166 files |
...
The core grew from 527 lines to 718 across six hundred and seventy seven features, and every
...
## The 218 it ships with

![The plugin rack: 218 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and seventy seven features in
one hundred and eighty phases, forty four shipped, six hundred and thirty new in the
