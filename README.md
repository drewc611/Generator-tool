...
lines of core, no runtime dependencies, 206 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 1149 passing, 1150 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the executable reader, the measured screenshot and the console's intake](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 1149
tests pass across 154 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 206 plugin(s)
...
| Every line of the tool | 54,980 lines of JavaScript in src and plugins |
| Tests | 19,928 lines, 1150 cases across 154 files |
| Source on disk | src 27 KB, plugins 2.6 MB |
...
The core grew from 527 lines to 718 across six hundred and sixty five features, and every
...
## The 206 it ships with

![The plugin rack: 206 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and sixty five features in
one hundred and sixty eight phases, forty four shipped, six hundred and eighteen new in the
