...
![node --test running the portamp suite: 793 passing, 794 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit, Alpine and Stencil readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 793
...
npm test                     # 794 tests, node --test, no framework
```
...
| Tests | 10,984 lines, 794 cases across 100 files |
...
The core grew from 527 lines to 718 across five hundred and eighty two features, and every
