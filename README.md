...
lines of core, no runtime dependencies, 105 plugins, and the literal output of
`npm run demo`.</sub>
...
node src/cli.js plugins      # 105 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 553 tests, node --test, no framework
```
...
| Every line of the tool | 23,078 lines of JavaScript |
| Tests | 6,880 lines, 553 cases |
...
![The plugin rack: 105 plugins listed by class, with what each one does](media/plugin-rack.svg)
