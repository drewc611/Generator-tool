...
lines of core, no runtime dependencies, 202 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 1108 passing, 1109 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the executable reader, the measured screenshot and the console's intake](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 1108
tests pass across 150 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 202 plugin(s)
...
| Every line of the tool | 53,192 lines of JavaScript in src and plugins |
| Tests | 19,019 lines, 1109 cases across 150 files |
| Source on disk | src 27 KB, plugins 2.5 MB |
...
The core grew from 527 lines to 718 across six hundred and sixty one features, and every
...
## The 202 it ships with

![The plugin rack: 202 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and sixty one features in
one hundred and sixty four phases, forty four shipped, six hundred and fourteen new in the
