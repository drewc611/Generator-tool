...
lines of core, no runtime dependencies, 170 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 827 passing, 828 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 827
tests pass across 108 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 170 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 828 tests, node --test, no framework
```
...
| Every line of the tool | 36,652 lines of JavaScript in src and plugins |
| Tests | 11,620 lines, 828 cases across 108 files |
...
The core grew from 527 lines to 718 across five hundred and ninety five features, and every
...
## The 170 it ships with

![The plugin rack: 170 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): five hundred and ninety five features in
one hundred and eleven phases, forty four shipped, five hundred and forty eight new in the
