...
![node --test running the portamp suite: 860 passing, 861 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 860
...
npm test                     # 861 tests, node --test, no framework
```
...
| Tests | 12,107 lines, 861 cases across 112 files |
...
The core grew from 527 lines to 718 across six hundred and two features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and two features in
one hundred and eighteen phases, forty four shipped, five hundred and fifty five new in the
