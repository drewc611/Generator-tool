...
![node --test running the portamp suite: 892 passing, 893 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 892
...
npm test                     # 893 tests, node --test, no framework
```
...
| Tests | 12,808 lines, 893 cases across 117 files |
...
The core grew from 527 lines to 718 across six hundred and eleven features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and eleven features in
one hundred and twenty seven phases, forty four shipped, five hundred and sixty four new in the
