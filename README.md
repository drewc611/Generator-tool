...
lines of core, no runtime dependencies, 156 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 759 passing, 760 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit, Alpine and Stencil readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 759
tests pass across 93 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 156 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 760 tests, node --test, no framework
```
...
| Every line of the tool | 33,488 lines of JavaScript in src and plugins |
| Tests | 10,438 lines, 760 cases across 93 files |
...
The core grew from 527 lines to 718 across five hundred and seventy five features, and every
...
## The 156 it ships with

![The plugin rack: 156 plugins listed by class, with what each one does](media/plugin-rack.svg)
