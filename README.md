...
lines of core, no runtime dependencies, 98 plugins, and the literal output of
`npm run demo`.</sub>
...
node src/cli.js plugins      # 98 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 543 tests, node --test, no framework
```
...
| Every line of the tool | 22,354 lines of JavaScript |
| Tests | 6,672 lines, 543 cases |
...
![The plugin rack: 98 plugins listed by class, with what each one does](media/plugin-rack.svg)
