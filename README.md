...
![node --test running the portamp suite: 906 passing, 907 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 906
tests pass across 119 test files with `node --test` and no framework, and
...
npm test                     # 907 tests, node --test, no framework
```
...
| Every line of the tool | 40,638 lines of JavaScript in src and plugins |
| Tests | 13,123 lines, 907 cases across 119 files |
...
The core grew from 527 lines to 718 across six hundred and fifteen features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and fifteen features in
one hundred and thirty one phases, forty four shipped, five hundred and sixty eight new in the
