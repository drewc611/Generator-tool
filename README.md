...
![node --test running the portamp suite: 659 passing, 660 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit and Alpine readers](media/test-run.png)
