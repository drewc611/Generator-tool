...
lines of core, no runtime dependencies, 180 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 917 passing, 918 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 917
tests pass across 121 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 180 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 918 tests, node --test, no framework
```
...
| Every line of the tool | 40,944 lines of JavaScript in src and plugins |
| Tests | 13,370 lines, 918 cases across 121 files |
| Source on disk | src 27 KB, plugins 1.8 MB |
...
The core grew from 527 lines to 718 across six hundred and nineteen features, and every
...
## The 180 it ships with

![The plugin rack: 180 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and nineteen features in
one hundred and thirty five phases, forty four shipped, five hundred and seventy two new in the
