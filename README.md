...
lines of core, no runtime dependencies, 158 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 770 passing, 771 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit, Alpine and Stencil readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 770
tests pass across 95 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 158 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 771 tests, node --test, no framework
```
...
| Every line of the tool | 34,067 lines of JavaScript in src and plugins |
| Tests | 10,614 lines, 771 cases across 95 files |
| Source on disk | src 27 KB, plugins 1.45 MB |
...
The core grew from 527 lines to 718 across five hundred and seventy seven features, and every
...
## The 158 it ships with

![The plugin rack: 158 plugins listed by class, with what each one does](media/plugin-rack.svg)
