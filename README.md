...
lines of core, no runtime dependencies, 155 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 755 passing, 756 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins including the transformer, the learned archetype model and the Svelte, Lit, Alpine and Stencil readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 755
tests pass across 92 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 155 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 756 tests, node --test, no framework
```
...
| Tests | 10,122 lines, 756 cases across 92 files |
...
## The 155 it ships with

![The plugin rack: 155 plugins listed by class, with what each one does](media/plugin-rack.svg)
