...
![node --test running the portamp suite: 886 passing, 887 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 886
...
npm test                     # 887 tests, node --test, no framework
```
...
| Tests | 12,680 lines, 887 cases across 116 files |
...
The core grew from 527 lines to 718 across six hundred and ten features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and ten features in
one hundred and twenty six phases, forty four shipped, five hundred and sixty three new in the
