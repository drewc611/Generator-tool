...
sidebar rather than a skin. Everything on the panel is real: 728
...
honest: there is nowhere in 728 lines to hide a special case for Angular.

| | |
| --- | --- |
| Core | **728 lines** across four files |
...
```bash
cat src/core/*.js src/cli.js | wc -l    # 728, and the suite fails if this table drifts
du -sh src plugins                      # the whole tool
```

The core grew from 527 lines to 728 across six hundred and ninety two features, and every
...
- **Main pane**, centre. The recorded screenshot and the emitted component with
  a scrubber between them, styled like a media player's transport rather
  than a plain divider. Drag it, or focus it and use the arrow keys: it
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and ninety two features in
one hundred and ninety five phases, forty four shipped, six hundred and forty five new in the
