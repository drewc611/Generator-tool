...
lines of core, no runtime dependencies, 103 plugins, and the literal output of
`npm run demo`.</sub>
...
node src/cli.js plugins      # 103 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 550 tests, node --test, no framework
```
...
| Every line of the tool | 22,848 lines of JavaScript |
| Tests | 6,813 lines, 550 cases |
...
![The plugin rack: 103 plugins listed by class, with what each one does](media/plugin-rack.svg)
