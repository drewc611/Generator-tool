...
![node --test running the portamp suite: 907 passing, 908 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 907
...
npm test                     # 908 tests, node --test, no framework
```
...
| Every line of the tool | 40,692 lines of JavaScript in src and plugins |
| Tests | 13,178 lines, 908 cases across 119 files |
...
The core grew from 527 lines to 718 across six hundred and sixteen features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and sixteen features in
one hundred and thirty two phases, forty four shipped, five hundred and sixty nine new in the
