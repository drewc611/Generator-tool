...
lines of core, no runtime dependencies, 177 plugins, and the literal output of
`npm run demo`.</sub>

![node --test running the portamp suite: 895 passing, 896 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 895
tests pass across 117 test files with `node --test` and no framework, and
...
node src/cli.js plugins      # 177 plugin(s)
npm run demo                 # runs the pipeline against example/legacy
npm test                     # 896 tests, node --test, no framework
```
...
| Every line of the tool | 39,798 lines of JavaScript in src and plugins |
| Tests | 12,867 lines, 896 cases across 117 files |
...
The core grew from 527 lines to 718 across six hundred and twelve features, and every
...
## The 177 it ships with

![The plugin rack: 177 plugins listed by class, with what each one does](media/plugin-rack.svg)
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and twelve features in
one hundred and twenty eight phases, forty four shipped, five hundred and sixty five new in the
