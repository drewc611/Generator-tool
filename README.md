...
![node --test running the portamp suite: 912 passing, 913 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 912
...
npm test                     # 913 tests, node --test, no framework
```
...
| Every line of the tool | 40,710 lines of JavaScript in src and plugins |
| Tests | 13,257 lines, 913 cases across 120 files |
...
The core grew from 527 lines to 718 across six hundred and seventeen features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and seventeen features in
one hundred and thirty three phases, forty four shipped, five hundred and seventy new in the
