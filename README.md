...
![node --test running the portamp suite: 926 passing, 927 tests, 0 failing, 1 skipped, grouped by the core staying framework blind, nine targets on one IR, the countable claims, and the newest plugins, from the transformer and the learned archetype model to the Ember, Mithril, Marko, Liquid, Twig and XSLT readers](media/test-run.png)

<sub>Proof, not a promise. Every line above is verbatim from `npm test`: 926
...
npm test                     # 927 tests, node --test, no framework
```
...
| Every line of the tool | 41,229 lines of JavaScript in src and plugins |
| Tests | 13,597 lines, 927 cases across 123 files |
...
The core grew from 527 lines to 718 across six hundred and twenty three features, and every
...
The whole picture is [ROADMAP.md](ROADMAP.md): six hundred and twenty three features in
one hundred and thirty nine phases, forty four shipped, five hundred and seventy six new in the
