  ...
  setup({ on, log }) {
    on("emit", async (ctx) => {
      ...
      for (const screen of ctx.screens) {
        ...
        await ctx.write(`src/features/${name}/${name}.vue`, COMPONENT({ name, props, emits, result, collection, screen }));
        emitted += 1;
      }
...
const COMPONENT = ({ name, props, emits, result, collection, screen }) => {
