 ...
 * not hold and a mixin block are named rather than approximated. The locals
 * a view reads are its inputs, taken from the expressions only.
 */
...
export function lowerText(text) {
  let out = ""; let i = 0;
  while (i < text.length) {
    if (text[i] === "\\" && (text.startsWith("#{", i + 1) || text.startsWith("!{", i + 1))) { out += text.slice(i + 1, i + 3); i += 3; continue; }
...
export function lowerAttrs(text, classes, id, note) {
  ...
  for (const raw of splitCommas(text.replace(/\n/g, " "))) {
    ...
    for (const item of splitWordsKeepingValues(raw)) {
      ...
      if (isString) { parts.push(`${name}="${lowerText(plain)}"`); continue; }
...
function splitWordsKeepingValues(text) {
  ...
  for (let i = 0; i < text.length; i += 1) {
    ...
    if (c === '"' || c === "'" || c === "`") quote = c;
      ...
      if (sawEq && /[?:+\-*/&|=<>,!]\s*$/.test(before)) continue;
      if (sawEq && !/^\s*[\w:@.-]+(\s*!?=|\s*,|\s*$)/.test(text.slice(i))) continue;
...
export function parseTree(source) {
  ...
  for (let n = 0; n < lines.length; n += 1) {
    ...
    const node = { indent, line: raw.trim(), children: [], n: n + 1 };
  ...
  const lowerChildren = (children) => {
    for (let idx = 0; idx < children.length; idx += 1) {
      ...
      if (line.startsWith("+")) {
        ...
        if (node.children.length) note(`+${cm[1]} was called with a block; the mixin's \`block\` is filled at its call and named here.`);
        ...
        const substitute = (n) => {
          ...
          if (n.line === "block") { n.line = ""; n.children = JSON.parse(JSON.stringify(node.children)); }
          n.children.forEach(substitute);
        };
        clone.forEach(substitute);
        ...
        lowerChildren(clone.filter((c) => c.line || c.children.length));
        continue;
      }
      ...
      if (cm) {
        ...
        if (kw === "each" || kw === "for") {
          ...
          if (em[2]) for (let k = at + 1; k < out.length; k += 1) out[k] = out[k].replace(new RegExp(`\\b${em[2]}\\b`, "g"), "$index");
        ...
        if (kw === "case") {
          const subject = line.slice(4).trim(); const tried = [];
          for (const w of node.children) {
            const wm = /^when\s+([\s\S]+?)(?::\s*(.*))?$/.exec(w.line);
            if (wm) {
              const test = `(${subject}) == ${wm[1].trim()}`;
              out.push(`<ng-container ng-if="${attrSafe(test)}">`); tried.push(test);
  ...
  setup({ on, log }) {
    on("extract", async (ctx) => {
      ...
      const resolve = (name) => {
        ...
        const key = [...bodies.keys()].find((k) => k === withExt || k.endsWith(`/${withExt}`)) ?? [...bodies.keys()].find((k) => k.endsWith(`/${withExt.split("/").pop()}`));
        return key ? bodies.get(key) : null;
      };
      const extended = new Set();
      for (const text of bodies.values()) for (const m of text.matchAll(/^\s*extends\s+(\S+)/gm)) extended.add(m[1].replace(/^\.?\//, "").replace(/\.pug$/, ""));
      ...
      for (const file of files) {
        ...
        if (!raw.trim()) continue;
        if ([...extended].some((p) => rel.replace(/\.pug$/, "") === p || rel.replace(/\.pug$/, "").endsWith(`/${p}`))) { note(`${rel} is a layout other templates extend; it is composed into each of them rather than ported as a screen of its own.`); continue; }
