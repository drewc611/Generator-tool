...
const DESCRIBES = [
  ...
  [/^DIALOGS\.md$/, "every dialog template an executable carried, each control with its class, id, caption and rectangle in dialog units"],
  [/^MENUS\.md$/, "every menu template as the tree it declares, with the command id each item sent"],
  [/^STRINGS\.md$/, "the executable's string table: the messages its code showed, by id, with no place in the port yet"],
  [/^PALETTE\.md$/, "the colours each PNG or JPEG screenshot is made of, counted from its pixels with their shares"],
  [/^WINFORMS\.md$/, "every Windows Forms designer file's controls: name, type, caption, position, size, tab index, anchor and the events wired"],
  [/^FORMS_VB6\.md$/, "every Visual Basic 6 form's controls, menus, message strings and each record read from its frx companion"],
  [/^RESOURCES\.md$/, "every resource script's dialogs, menus, strings, version block, accelerators and images, ids resolved through resource.h"],
  [/^PHOTO\.md$/, "every photographed screen's regions by shape and position, and the lines of writing that are inputs to fill"],
  [/^FORMS_DELPHI\.md$/, "every Delphi or Lazarus form's controls, menus and the data components the port must supply"],
  [/^LAYOUT\.md$/, "every XAML window or page's panel tree with its grid rows and columns, each control's name, type and bindings"],
  [/^FBP\.md$/, "every wxFormBuilder .fbp project's widget tree, what lowered and what was named as a gap"],
  [/^FETCH\.md$/, "what the site copy took, what it skipped and why, and the hosts the site leaned on"],
  ...
  [/^CICS\.md$/, "every CICS BMS .bms mapset's DFHMDI maps, one screen per map, with every gap named; no output, since BMS states none"],
  [/^COBOLSCREEN\.md$/, "every COBOL program's SCREEN SECTION, one screen per 01 level entry, with every gap named; no output, since a SCREEN SECTION states none"],
  ...
  [/^FLEX\.md$/, "every MXML Application read: what lowered onto the dialect, a {binding}, an unresolved dataProvider and an unmatched click handler each named as a gap"],
  [/^FLUID\.md$/, "every FLTK FLUID .fl designer file's root windows, one screen per window, with every gap named; radios grouped by shared immediate parent, a callback resolved only from a clean functionName(...) call"],
  ...
  [/^FXML\.md$/, "every JavaFX .fxml file's container and control tree, what lowered and what was named as a gap"],
  [/^GLADE\.md$/, "every GTK Builder .glade file's widget tree, what lowered and what was named as a gap"],
  [/^GWT\.md$/, "every GWT UiBinder view's widget tree, what lowered, and every button whose @UiHandler could not be matched"],
  ...
  [/^INFORMIX\.md$/, "every Informix 4GL/ESQL .per screen form's fields, read in the terminal's own row and column order, with every field on screen with no declared binding and every binding with no field named"],
  [/^INLINE\.md$/, "inline style and script counted per tag, for theming and a strict CSP"],
  [/^ISPF\.md$/, "every IBM ISPF Dialog Manager .panel definition's captions and fields, read in the terminal's own row and column order, with every attribute character this reader could not place named; no output, since a panel names none"],
  [/^JASPERREPORTS\.md$/, "every JasperReports .jrxml report's parameters, fields and bands, with the computed expressions, subreports and images it names rather than evaluates"],
  ...
  [/^NETBEANSFORM\.md$/, "every NetBeans Matisse .form file's component tree, what lowered and what was named as a gap"],
  [/^OBSERVERS\.md$/, "observers created and never disconnected"],
  [/^OPENEDGE\.md$/, "every OpenEdge ABL .p file's FORM screen declarations, fields and buttons in their own declared order, with a clean bare RUN wiring a real output and every gap named"],
  ...
  [/^PBWIN\.md$/, "every PowerBASIC .bas file's DIALOG NEW/CONTROL ADD statements, one screen per dialog, with every gap named; a control's field name comes from its own numeric id, a button's wiring from a trailing CALL clause"],
  ...
  [/^POWERBUILDER\.md$/, "every PowerBuilder .srw window's real, non-forward control blocks, what lowered and what was named as a gap"],
  ...
  [/^QT\.md$/, "every Qt Designer .ui file's widget tree, what lowered and what was named as a gap"],
  ...
  [/^SORT_MULTIHEAD\.md$/, "the sort task with several attention heads, compared"],
  [/^SPEC_COVERAGE\.md$/, "the OpenAPI document against the calls the app makes, both directions"],
  [/^SSRS\.md$/, "every SSRS .rdl report's parameters, dataset fields and sections, with the computed expressions, subreports and images it names rather than evaluates"],
  [/^STORAGE\.md$/, "localStorage, sessionStorage and IndexedDB keys the app reads and writes, values withheld"],
  [/^STORYBOARD\.md$/, "every Interface Builder .storyboard or .xib file's scenes, what lowered, what was named as a gap, and the segues found"],
  [/^STUDY\.md$/, "the study helper's solver demonstrated on one fixed equation, with the real steps that produced its answer"],
  [/^SUPPLYCHAIN\.md$/, "third party scripts and styles, and whether each carries an integrity hash"],
  [/^SWING\.md$/, "every Java Swing form's initComponents method, its widgets read and what was named as a gap"],
  [/^TABLES\.md$/, "data tables without headers, scope or captions"],
  [/^TAPESTRY\.md$/, "every Tapestry .tml template's t:type components, t:if and t:loop, with an unresolved select model, a submit button's convention bound handler and any component with no vocabulary entry named"],
  [/^TK\.md$/, "every Tcl/Tk script's widget-creation commands read in declaration order, radio buttons grouped by a shared -variable, and an unbound entry or checkbutton, a brace-quoted -command and an unrecognised widget command each named as a gap"],
  ...
  [/^UIKIT\.md$/, "every raw UIKit view construction .m file's controls, one screen per file, with every gap named; a button's wiring read from its own @selector(methodName)"],
  [/^UNO\.md$/, "every UNO dialog .xdl file's control tree, what lowered and what was named as a gap"],
  [/^WIDGETS\.md$/, "the jQuery UI and plugin widgets found, with what each becomes"],
  [/^XBASE\.md$/, "every xBase .prg file's @ row, col SAY/GET screens, one per READ, with a VALID, WHEN, RANGE or DEFAULT clause named rather than evaluated"],
